...
import fs from "fs";
import { config, uploaderPath } from "../components/config.js";

const router = Router();

router.use((req, res, next) => {
	if (!config.uploader) return res.status(500).json({error: "No uploader section in config!"});
	next();
})
...
router.post("/upload/", upload.single("file") ,async (req,res) => {
	if (!fs.existsSync(uploaderPath)) fs.mkdirSync(uploaderPath);
	...
	if (req.body.key !== config.masterKey) return res.status(403).json({error: "Wrong upload key!"});

	if (!req.file) return res.status(406).json({error: "No file provided!"});

	const file = req.file;
	const ext= path.extname(file.originalname);
	if (!config.uploader.extensions?.includes(ext.slice(1))) return res.status(415).json({error: "File format is not allowed!"});
	const md5sum = crypto.createHash("md5").update(file.buffer).digest();
	const filename = md5sum.toString("hex").slice(-10)+ext;
	if (!fs.existsSync(uploaderPath+filename)) fs.writeFileSync(uploaderPath+filename, file.buffer);
	const md5stamp = crypto.createHash("md5").update(fs.statSync(uploaderPath+filename).birthtime.getTime().toString()).digest("base64url");
	res.json({
		filename: filename,
...
router.get("/get/:filename", async (req, res) => {
	const file = uploaderPath + req.params.filename;
	const ext = path.extname(req.params.filename);
	if (!fs.existsSync(file)) return res.sendStatus(404);
	let buffer = fs.readFileSync(file);

	if (config.uploader.watermark?.enabled && !(load > config.uploader.watermark.maxLoad) && config.uploader.watermark.extensions?.includes(ext.slice(1)) && mime.getType(ext).includes("image")) {
		const size = imageSize(file);
		const canvas = Canvas.createCanvas(size.width, size.height);
		const ctx = canvas.getContext("2d");
		const background = new Canvas.Image();
		background.src = buffer;
		const watermark = await Canvas.loadImage(config.uploader.watermark.url);
		ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
		ctx.globalAlpha = 0.6
		const watermarkSize = (_ => {
			const min = canvas.width < canvas.height ? canvas.width : canvas.height;
			if (min * config.uploader.watermark.size.mult > config.uploader.watermark.size.max) return config.uploader.watermark.size.max
			else if (min * config.uploader.watermark.size.mult < config.uploader.watermark.size.min) return config.uploader.watermark.size.min
			else return min * config.uploader.watermark.size.mult
		})()
